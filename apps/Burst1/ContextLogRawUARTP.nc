...
implementation {
    ...
    message_t uart_msg;
    ctx_log_msg_t* log_data_msg;
    uint8_t len;

    static act_t m_ctx_idle;
    ...
    event void
    Boot.booted() {
        m_ctx_idle = mk_act_local(QUANTO_ACTIVITY(IDLE));
        call SerialControl.start();
        len = sizeof(ctx_log_msg_t);
    ...
    recordChange(uint8_t id, uint16_t value, uint8_t type) 
    {
        ...
        if (e) {
            ...
            e->type = type;
            e->res_id = id;
            e->ctx  = value; //also works for powerstate
        }
        if (m_state == S_FULL)
            post full();
    }
    ...
    async event void 
    SingleContextTrack.changed[uint8_t id](act_t new_context) 
    {
        recordChange(id, new_context, TYPE_SINGLE_CHG_NORMAL);
    }

    async event void 
    SingleContextTrack.bound[uint8_t id](act_t new_context) 
    {
        recordChange(id, new_context, TYPE_SINGLE_CHG_BIND);
    }

    async event void 
    SingleContextTrack.enteredInterrupt[uint8_t id](act_t new_context) 
    {
        recordChange(id, new_context, TYPE_SINGLE_CHG_ENTER_INT);
    }

    async event void 
    SingleContextTrack.exitedInterrupt[uint8_t id](act_t new_context) 
    {
        recordChange(id, new_context, TYPE_SINGLE_CHG_EXIT_INT);
    }
   
    async event void
    MultiContextTrack.added[uint8_t id](act_t context)
    {
        recordChange(id, context, TYPE_MULTI_CHG_ADD);
    }

    async event void
    MultiContextTrack.removed[uint8_t id](act_t context)
    {
        recordChange(id, context, TYPE_MULTI_CHG_REM);
    }

    async event void
    MultiContextTrack.idle[uint8_t id]()
    {
        recordChange(id, m_ctx_idle, TYPE_MULTI_CHG_IDL);
    }
    ...
    command error_t QuantoLog.flush() {
        ...
        memset(log_data_msg, 0, sizeof(ctx_log_msg_t));
