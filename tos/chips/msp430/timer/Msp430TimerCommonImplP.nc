...
implementation
{
  ...
  { 
    act_t ctx = call CPUContext.enterInterrupt(QUANTO_ACTIVITY(PXY_TIMERA0)); 
  ...
  { 
    ...
    act_t ctx = call CPUContext.enterInterrupt(QUANTO_ACTIVITY(PXY_TIMERA1)); 
#endif
  ...
  { 
    act_t ctx = call CPUContext.enterInterrupt(QUANTO_ACTIVITY(PXY_TIMERB0)); 
    signal VectorTimerB0.fired(); 
    call CPUContext.exitInterrupt(ctx);
  }
  TOSH_SIGNAL(TIMERB1_VECTOR)
  { 
    act_t ctx = call CPUContext.enterInterrupt(QUANTO_ACTIVITY(PXY_TIMERB1)); 
