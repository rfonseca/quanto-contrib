...
implementation {
    ...
    {
        if (!c) return QUANTO_ACTIVITY(UNKNOWN);
    ...
    inline async command void ActivityType.init(act_t *c) 
    {
        ...
        call ActivityType.setActType(c, QUANTO_ACTIVITY(UNKNOWN));
    }
    ...
    inline async command void ActivityType.setIdle(act_t *c)
    {
        if (!c) return;
        call ActivityType.setNode(c, TOS_NODE_ID);
        call ActivityType.setActType (c, QUANTO_ACTIVITY(IDLE));   
    }
