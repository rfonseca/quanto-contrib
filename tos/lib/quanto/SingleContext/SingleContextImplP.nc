...
implementation {
    ...
    command error_t Init.init() {
        ...
        ctx_local_unknown = mk_act_local(QUANTO_ACTIVITY(UNKNOWN));
        ctx_local_idle = mk_act_local(QUANTO_ACTIVITY(IDLE));
    ...
    async inline command void SingleContext.set[uint8_t res_id](act_t n) {
        ...
        if (n != old)
            signal SingleContextTrack.changed[res_id](old, n);
    }
    ...
    SingleContext.bind[uint8_t res_id](act_t n) {
        ...
        if (n != old)
            signal SingleContextTrack.bound[res_id](old, n);
    }
    ...
    SingleContext.enterInterrupt[uint8_t res_id](act_t n) 
    {
        ...
        signal SingleContextTrack.enteredInterrupt[res_id](old, n);
        return old;
    }
    ...
    SingleContext.exitInterrupt[uint8_t res_id](act_t restore_ctx) 
    {
        ...
        signal SingleContextTrack.exitedInterrupt[res_id](old, restore_ctx);   
    }
    ...
    SingleContext.exitInterruptIdle[uint8_t res_id]() 
    {
        ...
        signal SingleContextTrack.exitedInterrupt[res_id](old, ctx_local_idle);   
    }
    ...
    SingleContextTrack.changed[uint8_t res_id](act_t oldContext, act_t newContext)
    {
    }

    default async event void
    SingleContextTrack.bound[uint8_t res_id](act_t oldContext, act_t newContext) {
    }

    default async event void 
    SingleContextTrack.enteredInterrupt[uint8_t res_id](act_t oldContext, act_t newContext) {
    }

    default async event void 
    SingleContextTrack.exitedInterrupt[uint8_t res_id](act_t oldContext, act_t newContext) {
    }
