...
from QuantoCTimeMsg import QuantoCTimeMsg
...
def entriesFromCTimeFile(fin):
   """ Retuns an iterator over the entries in the open file fin.
       Expects a file with each line corresponding to a QuantoCTimeMsg 
       with NO AM HEADER
   """
   for line in fin:
        line = line.rstrip();
        ...
        try:
            ...
            continue
        try:
            entry = QuantoCTimeMsg( data=bytes, data_length = len(bytes))
        except Exception, x:
            print >>sys.stderr, x
            continue
        yield entry
